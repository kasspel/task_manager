...
class TaskListGroupActivity : AppCompatActivity() {
    private lateinit var toolbar: Toolbar
    private lateinit var currentProject: Project
    private lateinit var addTaskView: FloatingActionButton
    
    private lateinit var recycler: RecyclerView
    private val adapter = TaskNewAdapter()

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_task_list_group)
        ...
        when(Util.getUserStatus()){
            2 -> addTaskView.visibility = View.VISIBLE
            else -> addTaskView.visibility = View.GONE
        }
    ...
    override fun onCreateOptionsMenu(menu: Menu?): Boolean {
        ...
        if(!uid.equals(currentProject.oid)){
            menu?.findItem(R.id.delete_project)?.isVisible = false
            menu?.findItem(R.id.archive_project)?.isVisible = false
        }
        if(Repository.isArchive){
            menu?.findItem(R.id.archive_project)?.isVisible = false
            menu?.findItem(R.id.chat_project)?.isVisible = false
        }
        return true
    }
    ...
    private fun deleteProject(){
        ...
        builder.setPositiveButton(
            ...
            val pid = Repository.currentProject?.pid ?: ""
            if(pid.isNotEmpty()){
            ...
            if(pid.isNotEmpty()){
                ...
                db.collection("members")
                    ...
                    .addOnCompleteListener {
                        ...
                        for(doc in documents){
                            val id = doc.id
                            db.collection("members").document(id).update(data as Map<String, Any>)
                        }
                    }
    ...
    private fun infoProject(){
        val intent = Intent(this, InfoProjectActivity::class.java)
        startActivity(intent)
    }
    private fun chatProject(){
        val intent = Intent(this, ChatProjectActivity::class.java)
        startActivity(intent)
    }

    private fun loadTasks(){
        val pID = currentProject.pid
        val db = Firebase.firestore
        db.collection("tasks")
...
class TaskNewAdapter: ListAdapter<Task, TaskNewAdapter.ProjectViewHolder>(TaskComparator()) {
    ...
    class ProjectViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        private val photoView: ImageView = itemView.findViewById(R.id.photo_lead)
        private val nameLeadView: TextView = itemView.findViewById(R.id.name_lead)

        fun bind(current: Task, adapter: TaskNewAdapter) {
            val name = current.name
            val desc = current.desc
            val date = current.dateOfEnd
            val oid = Repository.currentProject?.oid ?:""
            ...
            val photo = current.leadPhoto
            val nameLead = current.leadName
        ...
        companion object {
            fun create(parent: ViewGroup): ProjectViewHolder {
                val view: View = LayoutInflater.from(parent.context)
                    .inflate(R.layout.item_task_new, parent, false)
